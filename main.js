(function (){
  ...
  for(let i = 600; i < 638; i++){
    ...
    input.type = 'checkbox';
    input.name = 'emojis';
    input.onclick = emojiCheck(input);
    label.append(input);
  }
...
function emojiCheck(input){
  if (input.checked){
          input.parentElement.classList.add('label_checked');
      }
      else{
        input.parentElement.classList.remove('label_checked');
      }
};
